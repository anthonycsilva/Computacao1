import random
def geradorCampo():
    '''essa função gera o campo minado e posiciona as minas de forma aleatória'''
    ''' -> list '''
...
def gerarMapaDeJogo():
    '''Essa função gera um mapa onde as casas são colocadas como "-"'''
    ''' -> list'''
...
def mapaJogo(mapa):
    '''Essa função organiza o mapa de forma horizontal alinhada para se parecer melhor com um campo minado'''
    ''' list '''
    for linha in mapa:
...
def verificacao(mapa):
    '''essa função verifica se a casa está ocultada ou não para dar continuidade ao jogo caso ainda n tenha terminado  '''
    ''' list '''
    for linha in mapa:
        for casa in linha:
            if casa == '-':
...
def continuar_jogo():
    '''essa função verifica a resposta do jogador, para que ele escolha se quer continuar o jogo ou não '''
    '''-> boolena'''
...
def main():
        '''essa função principal do jogo, ela opera as funções segundarias usando das informações fornecidas pela interação do usuário com o jogo'''
        mapa = geradorCampo()
        ...
        while True:
            if verificacao(mapa_do_jogador) == False:
                ...
                recusa = input('Digite "n" para sair: ')
                if x == '':
